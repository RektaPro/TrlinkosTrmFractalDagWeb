...
class TRLinkosMCPServer:
    ...
    def reason_step(
        ...
        """Execute a single reasoning step with strict validation.
        ...
        if len(x) != self.config["x_dim"]:
            return {
                "status": "error",
                "error": f"VALIDATION ERROR: x dimension mismatch. Expected {self.config['x_dim']}, got {len(x)}",
                "validation_failed": True,
            }
        ...
        if y is None:
            ...
            if len(y) != self.config["y_dim"]:
                return {
                    "status": "error",
                    "error": f"VALIDATION ERROR: y dimension mismatch. Expected {self.config['y_dim']}, got {len(y)}",
                    "validation_failed": True,
                }
        ...
        if z is None:
            ...
            if len(z) != self.config["z_dim"]:
                return {
                    "status": "error",
                    "error": f"VALIDATION ERROR: z dimension mismatch. Expected {self.config['z_dim']}, got {len(z)}",
                    "validation_failed": True,
                }
    ...
    def evaluate_score(
        ...
        """Evaluate prediction score with 100% accuracy and truthfulness.
